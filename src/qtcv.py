...
class Qtcv(QMainWindow, qtcvui.Ui_MainWindow):
    def __init__(self):
        # find the superclass of Qtcv and the super class run the __init__
    ...
    # the mouse release event of Qt
    def mouseReleaseEvent(self, QMouseEvent):
        x, y = QMouseEvent.pos().x(), QMouseEvent.pos().y()

        # use the Qt dialog to know how long the line in the real world
        if self.isCalibrating:
            self.calEnd = (int(x), int(y))
            referLen, ok = QInputDialog.getDouble(self, 'Input reference', 'Length (cm)')
            if ok and referLen:
                self.referLen = referLen
                # get the unit per pixel = refer len / the hypot
                self.unitPerPixel = referLen / float(np.hypot(self.calEnd[0] - self.calStart[0],self.calEnd[1] - self.calStart[1]))
                self.isCalibrating = False
                self.isCalibrated = True
        else:
            try:
                xmin, ymin, xmax, ymax = self.selection
            # avoid the bug of the user just click the mouse
            except TypeError as e:
                pass
            else:
                self.dragStart = None
                # get the startx, starty, xlength, ylength
